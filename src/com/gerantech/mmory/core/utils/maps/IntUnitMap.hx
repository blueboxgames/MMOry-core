...
{
	#if java
	public var _map:java.util.Map<Int,Unit>;
	#elseif flash
	public var _map:Map<Int,Unit>;
	#end

	public function new()
	{
		#if java
		_map = new java.util.concurrent.ConcurrentHashMap<Int,Unit>();
		#elseif flash
	...
	**/
	public function set(key:Int, value:Unit) : Void
	{
		#if java
		_map.put(key, value);
		#elseif flash
	...
	/**
		...
		If `key` is null, the result is unspecified.
	**/
	public function get(key:Int) : Unit
	{
		return _map.get(key);
	}
	...
	**/
	...
	{
		var keis:java.NativeArray<Dynamic> = _map.keySet().toArray();
		var ret:java.NativeArray<Unit> = new java.NativeArray<Unit>(keis.length);
		var i:Int = 0;
		while (i < keis.length)
		{
			ret[i] = cast(get(keis[i]), Unit);
			i++;
		}
		return ret ;
	}**/
